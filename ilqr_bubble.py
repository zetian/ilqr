    ...
    def cost(self):
        ...
        for i in range(self.horizon - 1):
            ...
            for obs in self.obs_list:
                ...
                cost += self.barrier_f(self.states[:, i], obs)
        ...
        for obs in self.obs_list:
                ...
                cost += self.barrier_f(self.states[:, -1], obs)
        return cost

    def barrier_f(self, x, obs):
        cost = 0.0
        for obs in self.obs_list:
            cost += self.obstacle_weight*np.exp(self.obstacle_weight_2*(obs[2]**2 - (x[0] - obs[0])**2 - (x[1] - obs[1])**2))
        return cost
